 ...
 */
@Deprecated
public class CopyOfNameResolver {
	...
	public CopyOfNameResolver(LocalVarStack localVars, Environment env) {
	...
	private VarDecl resolveFieldAccess(String uid, FileScope scope, RecordType curClass,ExprNode leftSide, Type leftType, SyntaxNode syntax, boolean usesSuper){
		//Is the type correct?
		RecordType r;
		VarDecl field;
		...
		if(!leftType.canHaveFields()){
			...
			if(field == null){
				...
				//Func pointer?
				Operation func = r.getMethods().resolveMethodInvocation(syntax, uid, null);
				if(func != null){
		...
		if(tryEnrichments){
			...
			if(field == null){
				...
				Operation func = es.getMethods().resolveInvocation(scope, syntax, uid, null);
				if(func != null){
					field = func.getPointerDecl(typeProvider);
				}
...
//			}
	...
	public VarDecl resolveVariable(FileScope scope,RecordType curClass,FieldAccessExprNode fieldAccess, boolean inMember){
		int accessType = fieldAccess.getInvocationType();
...
//			break;
	...
	private VarDecl resolveSimpleVariable(FileScope scope,RecordType curClass,FieldAccessExprNode name, boolean inMember){
		...
		if(curClass != null){
			...
			//Method?
			Operation func = curClass.getMethods().resolveMethodInvocation(name, uid, null);
			if(func != null){
	...
	public Invocation resolvePrefixedFunctionCall(FileScope scope, RecordType curClass, SyntaxNode where, Type leftType, Signature sig, String uid, boolean isStatic){
		//Is the type correct?
		RecordType r;
		Operation meth;
	...
	public Invocation resolveFunctionCall(FileScope scope,RecordType curClass,MethodInvocationExprNode methodInvocation, boolean inMember){
		...
		if(canBeMethod&&curClass != null){
			Operation m = curClass.getMethods().resolveMethodInvocation(methodInvocation, name, s);
			if(m!=null){
				if(m.isStatic())
