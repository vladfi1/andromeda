...
public class Extension extends BasicType{
	...
	public Extension(TypeExtension def, Type extendedType) {
		...
		case EXTENSION:
			...
			if(isDistinct)
				throw new CompilationError(def,"The base type of a disjoint type extension must be a native type, but it is a " + extendedType.getDescription());
			break;
		default:
			throw new CompilationError(def,"The type " + extendedType.getFullName() + " cannot be extended. Only basic types and extensions can be extended.");
		}
		definition = def;
		...
		if(isKey){
			if(extendedBaseType != BasicType.INT && extendedBaseType != BasicType.STRING){
				throw new CompilationError(def,"Only int and string are allowed to be used as base types for key types.");
			}
		}
