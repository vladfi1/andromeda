...
//  "strictfp"                     { return symbol(STRICTFP); }
  ...
  /* include */
  "include"{IncludeFile}   { Symbol s = processInclude(AndromedaFileInfo.TYPE_INCLUDE,false);
  							 if(s != null) return s; }
  "include"{LibIncludeFile} { Symbol s = processInclude(AndromedaFileInfo.TYPE_LIBRARY,false);
  							 if(s != null) return s; }
  "import"{LibImportFile} { Symbol s = processInclude(AndromedaFileInfo.TYPE_LIBRARY,true);
  							 if(s != null) return s; }
...
<STRING> {
  ...
  {LineTerminator}               { throw new CompilationError(curFile+yychar-string.length()-1,curFile+yychar,"Unterminated string at end of line"); }
}
